...
function getBooksFromGoogle(searchKey) {
  ...
  return axios.get(url)
    .then(res => _filterInfo(res.data.items.slice(0, 5)))

}

function _filterInfo(items) {
  if (!items) return
  ...
  items.map(item => {
    let filteredItem = {
      ...
      "thumbnail": (item.volumeInfo.imageLinks)? item.volumeInfo.imageLinks.thumbnail : '../../BooksImages/no-book.jpg',
      "listPrice": {
    ...
    filteredInfo.push(filteredItem)
